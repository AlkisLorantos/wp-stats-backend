...
export const updateGameController = async (
  ...
  try {
    const updated = await updateGame(Number(gameId), req.user.teamId, {
      date,
      opponent,
      location,
      teamScore,
...
export const deleteGameController = async (
  ...
  try {
    await deleteGame(Number(gameId), req.user.teamId);
...
export const startGameController = async (
  ...
  try {
    const game = await startGame(Number(gameId), teamId);
...
export const endGameController = async (
  req: AuthRequest,
  res: Response
): Promise<void> => {
  const { gameId } = req.params;
  ...
  try {
    const game = await endGame(Number(gameId), teamId);
